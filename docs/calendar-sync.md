...
2. 종료 시점이 현재보다 과거
3. 이벤트 `visibility`가 `public`, `default` 또는 미지정
4. 제목이 `[강의]`로 시작

출력 제목에서는 `[강의]`와 뒤따르는 공백을 제거한다. 맨 앞이나 맨 뒤 괄호는 기관명으로 분리한다. `visibility=default`는 캘린더의 기본 공개 설정을 따르는 일정으로 허용하고, `private`와 `confidential`은 제외한다. 취소된 일정은 제외한다.
...
앞으로 자동 공개할 일정은 제목 앞에 `[강의]`를 붙인다. 일정 공개 상태는 캘린더 기본값을 그대로 사용해도 되며, 명시적으로 `private` 또는 `confidential`인 일정은 제외한다. 기관명은 `[강의] 강의명 (기관명)` 또는 `[강의] (기관명) 강의명` 형식으로 적는다. 설명란은 비워도 되며 과거 수동 이력에는 접두어가 필요 없다.
